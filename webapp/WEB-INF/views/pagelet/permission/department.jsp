		...
		<c:when test="${pagedResult.totalCountOfElements > 0}">
			<c:forEach items="${pagedResult.contents}" var="smallGroup"
				...
				<tr>
					...
					<td><button class="btn btn-small hmo-button-khaki" data-map='{"id":"${smallGroup.id }", "type":"${smallGroup.type }" ,"name":"${smallGroup.name }"}' onclick="return add_candi_permission(this);">추가</button>
				</tr>
			</c:forEach>
		</c:when>
		<c:otherwise>
