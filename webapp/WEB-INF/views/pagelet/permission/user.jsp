	...
	<c:choose>
		<c:when test="${pagedResult.totalCountOfElements > 0}">
			<c:forEach items="${pagedResult.contents}" var="user"
				...
				<tr>
					<td>${user.name }</td>
					<td><button class="btn btn-small hmo-button-khaki" data-map='{"id":"${user.mySmallGroup.id }" , "type":${user.mySmallGroup.type }, "name":"${user.name }"}' onclick="return add_candi_permission(this);">추가</button>
				</tr>
			</c:forEach>
		</c:when>
		<c:otherwise>
