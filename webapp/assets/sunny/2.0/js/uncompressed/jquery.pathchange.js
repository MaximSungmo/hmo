  ...
  var my = $.pathchange = {
    ...
    init: function(options) {
      ...
      if (my.options.interceptLinks) {
        $("body").delegate('a[href^="/"]' + my.options.selector, "click", function(e) {
          my.changeTo($(this).attr("href"), this);
          e.preventDefault();
        });
      }
    ...
    changeTo: function(path, _this) {
      // If we're using History Management, just push an entry

    	// force trigger pathchange
    	// IE8에서 잘안먹어서 넣어줌.
    	$(window).trigger("pathchange", [_this]);
      ...
      if (my.options.useHistory && my.detectHistorySupport()) {
        ...
        return false;
        // 무슨 일이 있어도 이건 안됨. 왜냐면 이걸로 ajax 하고있거든.
