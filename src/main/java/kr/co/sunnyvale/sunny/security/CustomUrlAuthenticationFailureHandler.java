...
import org.springframework.util.Assert;
	...
	public class CustomUrlAuthenticationFailureHandler implements AuthenticationFailureHandler {
	    ...
	    public void onAuthenticationFailure(HttpServletRequest request, HttpServletResponse response,
	    	...
	    	if( accept != null && accept.matches( ".*application/json.*" ) == true ) {
	        	...
	        	if( exception instanceof InactiveUserException ){
	        		jsonResult = new JsonResult( false, "미인증 사용자입니다.", null );
		        	
		    	}else if( exception instanceof InactiveAdminException ){
		    		jsonResult = new JsonResult( false, "미인증된 관리자입니다..", null );
		    	}else{
		    		jsonResult = new JsonResult( false, "아이디가 존재하지 않거나 비밀번호를 틀렸습니다.", null );
		    	}
	    	...
	    	if(  exception.getCause() instanceof InactiveUserException ){
	    		saveException( request, exception );
	    		redirectStrategy.sendRedirect( request, response, "/user/reactivate?username=" + request.getParameter("username"));
	    	}else if( exception.getCause() instanceof InactiveAdminException ){
	    		saveException( request, exception );
//                request.getRequestDispatcher(  "/a/reactivate" ).forward( request, response );
	    		redirectStrategy.sendRedirect( request, response, "/a/reactivate?username=" + request.getParameter("username"));
	    	}else if( exception.getCause() instanceof InactiveUserNotYetAcceptInviteEmailException ){
	    		saveException( request, exception );
	    		redirectStrategy.sendRedirect( request, response, "/site/not_yet_invite?username=" + request.getParameter("username"));
	    	}
